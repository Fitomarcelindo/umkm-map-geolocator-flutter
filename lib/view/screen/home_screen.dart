...
import 'package:latlong2/latlong.dart' as latlng2; // Ganti LatLng dari Google Maps
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  static const latlng2.LatLng _initialPosition = latlng2.LatLng(-6.1650, 106.7260);

  // State untuk menyimpan koordinat pusat peta saat ini
  latlng2.LatLng _currentMapCenter = _initialPosition;
  final MapController _mapController = MapController(); // Controller untuk Flutter Map

  // Controller untuk input teks pencarian
  final TextEditingController _searchController = TextEditingController();
  bool _isSearching = false;
  Set<Marker> _umkmMarkers = {}; // Marker untuk menampilkan UMKM hasil pencarian
  ...
  Future<List<PlaceResult>> _fetchNearbyUmkmFromOverpass(latlng2.LatLng userLocation) async {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['elements'] != null) {
          ...
          for (var item in data['elements']) {
            final placeName = item['tags']['name'] as String? ?? 'UMKM Tanpa Nama';
        ...
        debugPrint('Overpass HTTP Error: ${response.statusCode}, Body: ${response.body}');
        throw Exception('Gagal memuat data dari Overpass API (${response.statusCode})');
      }
    } catch (e) {
  ...
  Future<void> _getCurrentLocationAndShowNearbyUmkm() async {
      ...
      if (permission == LocationPermission.deniedForever || permission == LocationPermission.denied) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Izin lokasi diperlukan untuk mencari UMKM terdekat.")),
          );
        }
        return;
      }
    ...
    try {
      ...
      final userLocation = latlng2.LatLng(position.latitude, position.longitude);

      // 3. Ambil data UMKM terdekat dari Overpass API
      final List<PlaceResult> nearbyUmkm = await _fetchNearbyUmkmFromOverpass(userLocation);

      // 4. Update Marker dan Pindahkan peta ke lokasi pengguna
      _updateUmkmMarkers(nearbyUmkm);
      _mapController.move(userLocation, 15.0); // Menggunakan move() dari MapController OSM
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Gagal mendapatkan lokasi atau data UMKM. Cek koneksi & Perangkat GPS Anda. Detail: ${e.toString().split(':')[0]}"),
  ...
  void _showNearbyUmkmDialog(List<PlaceResult> nearbyUmkm) {
    ...
    if (nearbyUmkm.isEmpty) {
      ...
      content = [
        const Text('Berikut daftar lokasi yang berada dalam radius 500m:', style: TextStyle(fontWeight: FontWeight.w600)),
        const Divider(),
        ...nearbyUmkm.map((place) => ListTile(
          dense: true,
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: Text(title, style: TextStyle(color: kPrimaryColor, fontSize: 18, fontWeight: FontWeight.bold)),
  ...
  void _searchLocation(String query) async {
    ...
    try {
      ...
      final response = await http.get(url, headers: {
        'User-Agent': 'UMKM_App_Geolocator_v1.0'
      });
      ...
      if (response.statusCode == 200) {
        ...
        if (data != null && data.isNotEmpty) {
          ...
          if (lat != null && lon != null) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text("Pencarian berhasil. Peta berpindah ke: $formattedAddress"),
                duration: const Duration(seconds: 3),
              ),
            );
          } else {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Format koordinat tidak valid dari Nominatim."), duration: Duration(seconds: 3)));
          }
        } else {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Lokasi tidak ditemukan oleh Nominatim."), duration: Duration(seconds: 3)));
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Error Nominatim HTTP: ${response.statusCode}"), duration: const Duration(seconds: 4)));
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Error Jaringan saat Pencarian: $e"), duration: const Duration(seconds: 4)));
    }
          ...
          options: MapOptions(
            ...
            onPositionChanged: (position, hasGesture) {
              if (position.center != null && !_isSearching) {
                ...
                debugPrint('Lokasi Terpilih (Tengah Peta): ${_currentMapCenter.latitude}, ${_currentMapCenter.longitude}');
              }
            },
            onMapReady: () {
